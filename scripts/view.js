...
export class View extends Bitmap
{
    ...
    drawLine(v0, v1)
    {
        ...
        if (m <= 1)
        {
            let y = 0;
            let z = 0;
            let delta1 = (p1.y - p0.y)/(p1.x-p0.x);
            let delta2 = (1/(p1.x - p0.x))*(1/(v1.pos.z) - 1/(v0.pos.z))
            for (let x = x0; x < x1; x++)
            {
                ...
                if(x == x0)
                {
                    ...
                    z = 1 / ((1 - per) / v0.pos.z + per / v1.pos.z);
        ...
        else
        {
            ...
            let x = 0;
            let z = 0;
            let delta1 = (p1.x - p0.x)/(p1.y - p0.y);
            let delta2 = (1/(p1.y - p0.y))*(1/(v1.pos.z) - 1/(v0.pos.z));
            for (let y = y0; y < y1; y++)
            {
                if(y == y0)
                {
